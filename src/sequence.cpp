...
std::expected<std::vector<float>, std::string> sample_motion_sequence_from_noise(
    ...
    for(size_t index=0;index<segments.size();++index){const auto&segment=segments[index];const size_t sampled_frames=segment.frames+(index?transition_frames:0);if(segment.frames<2||segment.embedding.size()!=4096||segment.initial_noise.size()!=sampled_frames*D)return std::unexpected("invalid sampled sequence segment");const bool user_conditioned=!segment.observed.empty()||!segment.observed_mask.empty();if(user_conditioned&&(segment.observed.size()!=sampled_frames*D||segment.observed_mask.size()!=sampled_frames*D))return std::unexpected("invalid sequence user constraints");std::vector<float>current;if(!index){auto sampled=user_conditioned?sample_motion_from_noise_conditioned(weights,segment.initial_noise,segment.embedding,segment.observed,segment.observed_mask,segment.heading,sampled_frames,steps,text_weight,constraint_weight):sample_motion_from_noise(weights,segment.initial_noise,segment.embedding,sampled_frames,steps,text_weight,constraint_weight);if(!sampled)return std::unexpected(sampled.error());current=std::move(*sampled);unnormalize(current);}else{const size_t overlap=transition_frames;if(overlap>=segment.frames||previous.size()<overlap*D)return std::unexpected("transition must be shorter than every following segment");auto transition=prepare_sequence_transition(weights,previous,segment.frames,transition_frames);if(!transition)return std::unexpected(transition.error());const float origin_x=transition->origin_x,origin_z=transition->origin_z;normalize(transition->observed);if(user_conditioned)for(size_t value=0;value<transition->observed.size();++value)if(segment.observed_mask[value]>0.5F){transition->observed[value]=segment.observed[value];transition->observed_mask[value]=segment.observed_mask[value];}auto sampled=sample_motion_from_noise_conditioned(weights,segment.initial_noise,segment.embedding,transition->observed,transition->observed_mask,segment.heading_constrained?segment.heading:transition->first_heading,sampled_frames,steps,text_weight,constraint_weight);if(!sampled)return std::unexpected(sampled.error());current=std::move(*sampled);unnormalize(current);for(size_t frame=0;frame<sampled_frames;++frame){auto*row=current.data()+frame*D;row[0]+=origin_x;row[2]+=origin_z;}const size_t start=joined.size()-overlap*D;for(size_t frame=0;frame<overlap;++frame){const float alpha=overlap==1?.5F:1.F-float(frame)/float(overlap-1);for(size_t d=0;d<D;++d)joined[start+frame*D+d]=alpha*joined[start+frame*D+d]+(1.F-alpha)*current[frame*D+d];}joined.insert(joined.end(),current.begin()+static_cast<std::ptrdiff_t>(overlap*D),current.end());}if(!index)joined=current;previous=std::move(current);}
    return joined;
}
} // namespace kimodo::detail
